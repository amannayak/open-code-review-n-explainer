...
func setConfigValue(cfg *Config, key, value string) error {
	...
	case "llm.allow_local_no_token", "llm.AllowLocalNoToken":
		b, err := strconv.ParseBool(value)
		if err != nil {
			return fmt.Errorf("invalid boolean for llm.allow_local_no_token: %w", err)
		}
		cfg.Llm.AllowLocalNoToken = b
	...
	case "llm.extra_body", "llm.ExtraBody":
		...
		cfg.Llm.ExtraBody = m
	default:
		return fmt.Errorf("unknown config key: %s\nSupported keys: llm.url, llm.auth_token, llm.model, llm.use_anthropic, llm.allow_local_no_token, llm.extra_body, language, telemetry.enabled, telemetry.exporter, telemetry.otlp_endpoint, telemetry.content_logging", key)
	}
	return nil
}
